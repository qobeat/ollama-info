...
ollama_models() { if __ollama_api_ready; then if __ollama_have_cmd ollama-status; then OLLAMA_MODEL_COMMAND="ollama bench" command ollama-status --models; else command ollama list; fi; else echo "Ollama API is not reachable at ${OLLAMA_URL:-http://127.0.0.1:11434}."; if __ollama_system_service_exists; then echo "Start: sudo systemctl start ollama"; else echo "Start: ollama serve"; fi; return 3; fi; }
...
ollama_bench() { local pattern="${1:-}"; if [ -z "$pattern" ]; then echo "Usage: ollama bench <model-pattern> [extra options]"; echo "Example: ollama bench qwen3-embedding:4b"; return 2; fi; shift || true; if __ollama_have_cmd ollama-bench-RTX3090.sh; then command ollama-bench-RTX3090.sh "$pattern" "$@"; else echo "ERROR: ollama-bench-RTX3090.sh not found. Add ~/dev/ollama-info/scripts to PATH." >&2; return 127; fi; }
...
if [ "${OLLAMA_BASHRC_WRAP_CLI:-1}" = "1" ] && __ollama_have_cmd ollama; then
  ollama() { local __sub="${1:-}"; case "$__sub" in status) shift; ollama_status "$@" ;; start) shift; ollama_start "$@" ;; stop) shift; ollama_stop "$@" ;; logs|log) shift; ollama_logs "$@" ;; models) shift; ollama_models "$@" ;; gpu) shift; ollama_gpu "$@" ;; test) shift; ollama_test "$@" ;; bench) shift; ollama_bench "$@" ;; embed-test|embedtest|embedding-test) shift; ollama_embed_test "$@" ;; *) command ollama "$@" ;; esac; }
fi
alias os='ollama_status'; alias oq='ollama_quick_status'; alias ost='ollama_start'; alias osp='ollama_stop'; alias om='ollama_models'; alias og='ollama_gpu'; alias ol='ollama_logs'; alias ot='ollama_test'; alias ob='ollama_bench'; alias oet='ollama_embed_test'
