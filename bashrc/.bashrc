...
ollama_test() { local pattern="${1:-}"; if [ -z "$pattern" ]; then echo "Usage: ollama test <model-pattern> [extra options]"; echo "Example: ollama test qwen3.6 --no-conc"; return 2; fi; shift || true; if __ollama_have_cmd ollama-test-and-monitor-RTX3090.sh; then command ollama-test-and-monitor-RTX3090.sh "$pattern" "$@"; else echo "ERROR: ollama-test-and-monitor-RTX3090.sh not found. Add ~/dev/ollama-info/scripts to PATH." >&2; return 127; fi; }

if [ "${OLLAMA_BASHRC_WRAP_CLI:-1}" = "1" ] && __ollama_have_cmd ollama; then
  ollama() { local __sub="${1:-}"; case "$__sub" in status) shift; ollama_status "$@" ;; start) shift; ollama_start "$@" ;; stop) shift; ollama_stop "$@" ;; logs|log) shift; ollama_logs "$@" ;; models) shift; ollama_models "$@" ;; gpu) shift; ollama_gpu "$@" ;; test) shift; ollama_test "$@" ;; *) command ollama "$@" ;; esac; }
fi
alias os='ollama_status'; alias oq='ollama_quick_status'; alias ost='ollama_start'; alias osp='ollama_stop'; alias om='ollama_models'; alias og='ollama_gpu'; alias ol='ollama_logs'; alias ot='ollama_test'
