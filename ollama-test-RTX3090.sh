...
VERSION="0.5.0"
SCRIPT_SIGNATURE="OLLAMA_TEST_RTX3090_SCRIPT_SIGNATURE=v0.5.0-health-performance"
...
PROMPT_PREFIX="${PROMPT_PREFIX:-/no_think}"
...
Options:
  ...
  --prompt-prefix TEXT  Prefix prompts; default '/no_think' helps Qwen3 reach response tokens
  --zip / --no-zip      Create ~/tmp archive on exit (default: $ZIP_ON_EXIT)
  -h, --help            Show help

Outputs:
  $RUN_DIR/
    meta.txt, summary.csv, summary.md, raw/*.json, payloads/*.json,
    ollama-ps-before.txt, ollama-ps-after.txt, nvidia-smi-before.txt, nvidia-smi-after.txt
EOF_USAGE
}
...
ensure_server() {
  if curl -fsS --connect-timeout "$CONNECT_TIMEOUT_SEC" "$BASE_URL/api/tags" >/dev/null 2>&1; then
    return 0
  fi
  ...
  if ! curl -fsS --connect-timeout "$CONNECT_TIMEOUT_SEC" "$BASE_URL/api/tags" >/dev/null 2>&1; then
    echo "ERROR: Ollama server still not reachable at $BASE_URL" >&2
    exit 3
  fi
}
...
model_available() {
  curl -fsS "$BASE_URL/api/tags" \
    | jq -e --arg m "$MODEL" '.models[]? | select(.name==$m or .model==$m)' >/dev/null
}
...
write_meta() {
  {
    ...
    echo
    echo "## API tags selected model"
    curl -fsS "$BASE_URL/api/tags" 2>/dev/null | jq --arg m "$MODEL" '.models[]? | select(.name==$m or .model==$m)' || true
    echo
    echo "## nvidia-smi -L"
...
snapshot_before_after() {
  ...
  if command -v ollama >/dev/null 2>&1; then
    ollama ps >"$RUN_DIR/ollama-ps-$label.txt" 2>&1 || true
  else
    echo "ollama CLI not found" >"$RUN_DIR/ollama-ps-$label.txt"
  fi
...
build_payload() {
  local prompt="$1" np="$2" ctx="$3" mode="$4"
  local gpu_opts='{}'
  if [[ "$mode" == "CPU" ]]; then
    gpu_opts='{"num_gpu":0}'
  fi
  jq -nc \
    ...
    --argjson gpu_opts "$gpu_opts" \
    '{model:$model,prompt:$prompt,stream:false,keep_alive:$keep,options:({num_predict:$np,num_ctx:$ctx,temperature:$temp,seed:42} + $gpu_opts)}'
}
...
append_summary_from_json() {
  ...
  if [[ -n "$err_msg" ]]; then
    jq -rn \
      --arg ts "$(date -Is)" --arg test "$test_name" --arg model "$MODEL" --arg ctx "$ctx" --arg np "$np" --arg mode "$mode" --arg conc "$conc" --arg err "$err_msg" --arg raw "$raw_file" \
  ...
  jq -r \
    ...
    [
      ...
      "",
      $raw
    ] | @csv' "$raw_file" >>"$SUMMARY_CSV"
}

run_generate() {
  local test_name="$1" mode="$2" conc="$3" np="$4" ctx="$5" prompt="$6"
  local payload_file="$PAYLOAD_DIR/$test_name.json"
  local raw_file="$RAW_DIR/$test_name.json"
  local stderr_file="$RAW_DIR/$test_name.stderr"
  local payload rc=0
  payload="$(build_payload "$prompt" "$np" "$ctx" "$mode")"
  printf '%s\n' "$payload" >"$payload_file"
  log "--- $test_name mode=$mode ctx=$ctx predict=$np conc=$conc ---"
  if ! timeout -k 10s "$TIMEOUT_SEC" curl -sS --fail-with-body \
      --connect-timeout "$CONNECT_TIMEOUT_SEC" \
      --max-time "$TIMEOUT_SEC" \
      -H 'Content-Type: application/json' \
      -H 'Accept: application/json' \
      -d "@$payload_file" \
      "$BASE_URL/api/generate" >"$raw_file" 2>"$stderr_file"; then
    rc=$?
    warn "$test_name failed rc=$rc stderr=$(tr '\n' ' ' <"$stderr_file" | cut -c1-300)"
    append_summary_from_json "$test_name" "$mode" "$conc" "$np" "$ctx" "$raw_file" "curl_failed_rc_$rc"
    return 0
  fi
  if ! jq -e . "$raw_file" >/dev/null 2>&1; then
    warn "$test_name returned non-JSON"
    append_summary_from_json "$test_name" "$mode" "$conc" "$np" "$ctx" "$raw_file" "non_json_response"
    return 0
  fi
  append_summary_from_json "$test_name" "$mode" "$conc" "$np" "$ctx" "$raw_file"
  jq -r '"done_reason=" + (.done_reason // "") + " eval_tokens=" + ((.eval_count // 0)|tostring) + " gen_tps=" + (if (.eval_duration // 0) > 0 then ((.eval_count / (.eval_duration/1000000000))|tostring) else "n/a" end) + " response_chars=" + (((.response // "")|length)|tostring) + " thinking_chars=" + (((.thinking // "")|length)|tostring)' "$raw_file" || true
}

make_summary_md() {
  {
    echo "# RTX 3090 Ollama Test Summary"
    echo
    echo "## Run metadata"
    ...
    awk -F',' '
      ...
      {
        test=unq($(h["test"])); mode=unq($(h["mode"])); ctx=unq($(h["num_ctx"])); np=unq($(h["num_predict"])); reason=unq($(h["done_reason"])); err=unq($(h["error"]))
        gen=unq($(h["gen_tps"])); eval=unq($(h["eval_tokens"])); resp=unq($(h["response_chars"])); think=unq($(h["thinking_chars"])); total=unq($(h["total_s"]))
        if(NR==2){print "| Test | Mode | Ctx | Predict | Eval tokens | Gen tok/s | Total s | Response chars | Thinking chars | Done | Error |"; print "|---|---|---:|---:|---:|---:|---:|---:|---:|---|---|"}
        if(gen!="") gen=sprintf("%.2f", gen); if(total!="") total=sprintf("%.2f", total)
        printf "| %s | %s | %s | %s | %s | %s | %s | %s | %s | %s | %s |\n", test, mode, ctx, np, eval, gen, total, resp, think, reason, err
      }' "$SUMMARY_CSV"
    echo
    echo "## Automatic interpretation"
    echo
    awk -F',' '
      function unq(s){gsub(/^"|"$/, "", s); gsub(/""/, "\"", s); return s}
      NR==1{for(i=1;i<=NF;i++) h[unq($i)]=i; next}
      {
        mode=unq($(h["mode"])); err=unq($(h["error"])); gen=unq($(h["gen_tps"]))+0; resp=unq($(h["response_chars"]))+0; think=unq($(h["thinking_chars"]))+0
        if(err!="") errors++
        if(mode=="GPU" && gen>0){gpu_n++; gpu_sum+=gen; if(gpu_n==1 || gen>gpu_max) gpu_max=gen; if(gpu_n==1 || gen<gpu_min) gpu_min=gen}
        if(mode=="CPU" && gen>0){cpu_n++; cpu_sum+=gen}
        if(resp==0 && think>0) thinking_only++
      }
      END{
        if(gpu_n>0) printf "- GPU generation speed: avg %.2f tok/s, min %.2f, max %.2f across %d GPU rows.\n", gpu_sum/gpu_n, gpu_min, gpu_max, gpu_n; else print "- GPU generation speed: no valid GPU rows."
        if(cpu_n>0) printf "- CPU generation speed: avg %.2f tok/s across %d CPU rows.\n", cpu_sum/cpu_n, cpu_n
        if(errors>0) printf "- Errors recorded: %d. Check errors.log and raw/*.stderr.\n", errors; else print "- Errors recorded: 0."
        if(thinking_only>0) printf "- Thinking-only responses: %d. This is common with Qwen3 if num_predict is consumed before final response; use /no_think or increase num_predict.\n", thinking_only
      }' "$SUMMARY_CSV"
    echo
    echo "## Ollama loaded models after test"
    echo '```text'
    cat "$RUN_DIR/ollama-ps-after.txt" 2>/dev/null || true
    echo '```'
    echo
    echo "## NVIDIA snapshot after test"
    echo '```text'
    cat "$RUN_DIR/nvidia-smi-after.txt" 2>/dev/null || true
    echo '```'
    echo
    echo "## Files"
    echo "- CSV: $SUMMARY_CSV"
    echo "- raw JSON: $RAW_DIR"
    echo "- payload JSON: $PAYLOAD_DIR"
    echo "- meta: $META"
  } >"$SUMMARY_MD"
}
...
Write a concise technical answer about GPU offload for local LLM inference. Use clear bullet points. Do not include hidden reasoning."
sanity_prompt="$PROMPT_PREFIX
Return exactly this text and nothing else: RTX 3090 Ollama test OK"
long_prompt="$PROMPT_PREFIX
Write a technical diagnostic memo about RTX 3090 local LLM inference in WSL2. Cover VRAM, CUDA offload, PCIe, thermals, power, prompt throughput, generation throughput, model loading, and failure signals."

run_generate "01_sanity_gpu" "GPU" "1" 128 "$NUM_CTX" "$sanity_prompt"
run_generate "02_throughput_gpu" "GPU" "1" "$NUM_PREDICT" "$NUM_CTX" "$base_prompt"
run_generate "03_sustained_gpu" "GPU" "1" "$LONG_NUM_PREDICT" "$NUM_CTX" "$long_prompt"
run_generate "04_longctx_gpu" "GPU" "1" "$NUM_PREDICT" "$LONG_CTX" "$long_prompt"

if [[ "$RUN_CONC" == "1" && "$CONCURRENCY" -gt 1 ]]; then
  log "--- concurrency probe: $CONCURRENCY parallel GPU requests ---"
  pids=()
  for i in $(seq 1 "$CONCURRENCY"); do
    run_generate "05_conc${CONCURRENCY}_req${i}" "GPU" "$CONCURRENCY" "$NUM_PREDICT" "$NUM_CTX" "$base_prompt" &
    pids+=("$!")
  done
  wait "${pids[@]}" || true
fi
...
if [[ "$RUN_CPU" == "1" ]]; then
  run_generate "06_cpu_reference" "CPU" "1" 128 "$NUM_CTX" "$base_prompt"
fi
